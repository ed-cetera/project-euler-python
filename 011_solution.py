...
# Main
def main():
    square = [[ 8,  2, 22, 97, 38, 15,  0, 40,  0, 75,
    ...
    for i in range(len(square) - 3):
        for j in range(len(square[0]) - 3):
            product = max(product, (square[i][j] * square[i][j+1]
                                    * square[i][j+2] * square[i][j+3]))
            product = max(product, (square[i][j] * square[i+1][j]
                                    * square[i+2][j] * square[i+3][j]))
            product = max(product, (square[i][j] * square[i+1][j+1]
                                    * square[i+2][j+2] * square[i+3][j+3]))
            product = max(product, (square[i+3][j] * square[i+2][j+1]
                                    * square[i+1][j+2] * square[i][j+3]))
